...
import Com.Gmail.Users.Config.Report;
 
public class UserInteraction2 extends Report {
	WebDriver driver;
	ExtentTest test;
	WebDriverWait wait;
	...
	/*public boolean Run(String expectedResult) {
		...
		try {
			...
			actResult = false;
			Report.generateReport(driver, test, Status.FAIL, "Something went wrong");
		}
		return actResult;
	}*/
	public boolean run(String expectedResult) {
		...
		try {
			...
			if(ele.getText().matches("Login Successfully")){
	   			Report.generateReport(driver, test, Status.PASS, "Valid User");
	   		 }
	   		 else
	   		 {
	   			actResult = false;
	            Report.generateReport(driver, test, Status.FAIL, "Invalid Credentials");
	   		 }
		}catch (Exception te) {
            actResult = false;
            Report.generateReport(driver, test, Status.FAIL, "Wait time exceeded...");
        }
